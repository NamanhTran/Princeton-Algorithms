...
public class RandomizedQueue<Item> implements Iterable<Item> {
    ...
    public void enqueue(Item item) {
        if (item == null) 
            throw new java.lang.IllegalArgumentException("The item null cannot be enqueued");
    ...
    public Item dequeue() {
        ...
        Item item = queue[random % queue.length];
    ...
    private class IterableQueue implements Iterator<Item> {
        private Item[] itQueue = queue.clone();
        ...
        public Item next() {
            ...
            if (firstRun) {
                ...
                if (itFront % itQueue.length < itEnd % itQueue.length) {
                    ...
                    endIndex = itEnd % itQueue.length;
                }

                else {
                    start = itEnd % itQueue.length;
                    endIndex = itFront % itQueue.length;
                }

                // Shuffle the array once
                StdRandom.shuffle(itQueue, start, endIndex);
                firstRun = false;
            }
    ...
    private void print() {
        System.out.println("Start: " + front + " End: " + end);
